...
## `HomeView/`
Default landing page (`syllabusView='dashboard'`). Surfaces learning stats and weekly goals for lightweight gamification.

### `HomeView/index.jsx`
Hero stats row (4 cards: streak, words learned, minutes read, retention rate) using `computeStats()`. Weekly Goals section via `WeeklyGoals` subcomponent. Continue Learning section showing most recent active course/reader. Quick action buttons for flashcards, create reader, and view stats.

### `WeeklyGoals.jsx`
Progress bars for 4 weekly goals (lessons, flashcard reviews, quizzes, minutes studied). Reads `weeklyGoals` + `learningActivity` + `readingTimeLog` from state. Uses `computeWeeklyProgress()` from `stats.js`. Edit toggle shows number inputs to adjust targets, dispatches `setWeeklyGoals()`.
