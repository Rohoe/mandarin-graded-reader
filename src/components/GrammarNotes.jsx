...
export default function GrammarNotes({ grammarNotes }) {
      ...
      {!collapsed && (
        <div className="grammar-notes__cards fade-in">
          {grammarNotes.map((note, i) => (
            <GrammarCard key={i} note={note} index={i} />
          ))}
        </div>
      )}
    </section>
...
function GrammarCard({ note, index }) {
  return (
    <div className="grammar-card">
      <div className="grammar-card__header">
        <span className="grammar-card__num">{index + 1}</span>
        <span className="grammar-card__pattern text-chinese">{note.pattern}</span>
      ...
      <div className="grammar-card__body">
        ...
        {note.example && (
          <p className="grammar-card__example text-chinese">{note.example}</p>
        )}
      </div>
    </div>
  );
