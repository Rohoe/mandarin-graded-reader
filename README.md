...
- **Romanization toggle** — 拼 (Chinese), 粵 (Cantonese), or Aa (Korean) icon button shows romanization above every character using `<ruby>` tags in both the story and comprehension questions. Powered by `pinyin-pro` for Chinese, `to-jyutping` for Cantonese, `hangul-romanization` for Korean
