...
- **Grammar Notes** — Each reader includes 3–5 collapsible grammar pattern cards explaining structures used in the story, with examples. Grammar patterns feed into a dedicated **Grammar SRS** review mode with independent spaced repetition tracking
...
- **Comprehension grading** — Type answers to comprehension questions and get AI-powered feedback with per-question scores; every question includes a toggleable suggested model answer; results persist across sessions. "Translate All" toggle translates questions to your native language via Google Translate
...
- **Flashcard review** — Built-in SRS flashcard system with multiple modes: **SRS Review** (vocab + grammar spaced repetition), **Quiz Mix**, **Practice**, **Sentence Builder**, **Context Clue**, and **Reverse Listening**. Mode picker UI for selecting review type. Forward cards (target→EN) and reverse cards (EN→target) with independent spaced repetition tracking. Reverse cards unlock after a word is reviewed forward at least once. Missed/almost cards are re-queued until all are "Got it". Configurable new cards per day (default 20). SRS interval previews below judgment buttons show upcoming intervals. Romanization ruby annotations on card front when enabled. Undo (Ctrl+Z) restores the previous card. Example sentences and translations shown on card backs. Sessions persisted per-language; resume on the same day, reset at midnight. "New session" resets the new-card budget and shows only never-reviewed cards (skips due cards entirely). Filter by language. Accessible from the sidebar footer or Stats dashboard
...
- **Home dashboard** — Default landing page with weekly reading goals, reading session logs, and activity overview. Reading time tracked automatically per session
- **Inline topic suggestions** — AI-generated topic suggestions shown as clickable chips during syllabus creation for quick inspiration
- **Markdown streaming** — Reader text renders as markdown progressively during streaming with debounced buffer updates, so you can read while the story is still being generated
- **Listening quiz** — Audio-based quiz mode with slow playback (0.75×) and hint buttons for guided practice
...
- **657 unit tests** (Vitest) — parser, reducer, stats, anki, storage, cloudSync, useReaderGeneration, grammarMapper, vocabMapper, chatApi, tutorPrompt, and more
- **48 E2E tests** (Playwright, 24 specs × 2 browser projects) — demo reader, settings, reader generation, flashcards, streaming, accessibility, undo/delete, mobile layout
